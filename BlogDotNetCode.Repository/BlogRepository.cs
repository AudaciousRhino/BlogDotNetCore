using BlogDotNetCode.Models.Blog;
using Dapper;
...
namespace BlogDotNetCode.Repository
{
    public class BlogRepository : IBlogRepository
    {
        ...
        public async Task<List<Blog>> GetAllFamousAsync()
        {
            ...
            using (var connection = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                ...
                famousBlogs = await connection.QueryAsync<Blog>(
                    "Blog_GetAllFamous",
                    new { }, //No parameters needed here, just passing in an empty list
                    commandType: CommandType.StoredProcedure);
            }
        ...
        public async Task<Blog> UpsertAsync(BlogCreate blogCreate, int applicationUserId)
        {
            ...
            newBlogId = newBlogId ?? blogCreate.BlogId; //newBlogId = newBlogId 'Null Coalesced' to blogCreate.(original)BlogId;
                                                        //i.e. if Insert, grab newBlogId | if Update, grab original BlogId
